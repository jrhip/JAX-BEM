...
if __name__ == "__main__":
    ...
    resolution = 128
    eta = 1j
    N = [2, 3, 4, 5]  # bempp regular_sphere refinement levels → [128, 512, 2048, 8192] elements
    ...
    bp_domain_mae = np.zeros((len(N), resolution, resolution, resolution))
    jax_domain_mae = np.zeros((len(N), resolution, resolution, resolution))
    rel_domain_mae = np.zeros(len(N))

    bempp_time = np.zeros(len(N))
    jax_time = np.zeros(len(N))
    ...
    for index in range(len(N)):
        ...
        mesh = bempp_cl.api.shapes.regular_sphere(int(N[index]))
        ...
        # JAX solve
        (jax_lhs, jax_rhs,
         jax_boundary_solution, jax_domain_solution, jax_time[index]) = bem_solve(
            k0=k0,
            vertices=vertices,
            elements=elements,
            normals=normals,
        ...
        bempp_time[index] = bp_toc - bp_tic
        
        print("bempp assembly: ",round(assembly_time,3)," seconds")
        print("bempp total: ",round(bempp_time[index],3)," seconds")
        
        print("Speedup: ",round(bempp_time[index]/jax_time[index],3),"x")
        ...
        rel_domain_mae[index,...] = np.mean(np.abs(jax_domain_solution - bp_domain_solution))
        
        print('Relative MAE ',rel_domain_mae)
    ...
    #%% Plot error (with error bars) against N

    # Element counts corresponding to regular_sphere refinement levels [2,3,4,5]
    N_elems = [128, 512, 2048, 8192]
    ...
    plt.loglog(N_elems, median, label='bempp')
    plt.fill_between(N_elems, lower, upper, alpha=0.2)
    plt.loglog(N_elems, median2, label='JAX-BEM')
    plt.fill_between(N_elems, lower2, upper2, alpha=0.2)
    plt.xlabel('N')
    plt.ylabel('MAE')
    plt.xlim(100, 10000)
    plt.ylim(1e-4, 1e0)
    ...
    plt.tight_layout()
    plt.savefig('plots/error_n.pdf', format='pdf')
    plt.show()

    plt.rcParams['figure.figsize'] = (8*cm, 7*cm)
    plt.loglog(N_elems, bempp_time, label='bempp')
    plt.loglog(N_elems, jax_time, label='JAX-BEM')
    plt.xlabel('N')
    plt.ylabel('Time [s]')
    plt.xlim(100, 10000)
    plt.ylim(1e0, 1e2)
    ...
    plt.tight_layout()
    plt.savefig('plots/time_n.pdf', format='pdf')
    plt.show()
